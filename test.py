...
def main():
    ...
    with open("./training-1.txt", "r") as f:
        ...
        for i in range(days):
            ...
            outPMTypeAll = '(purchase, %d)'
            outPMType = '(%s, %d)'
            outMigration = '(migration, %d)'
            outDeploySingle = '(%d, %s)'
            outDeployDouble = '(%d)'

            needtoBuy = len(pm2vm) - numPM
            if needtoBuy == 0:
                print(outPMTypeAll % 0)
            else:
                print(outPMTypeAll % 1)
            
                print(outPMType % (targetPM,needtoBuy))
            numPM = len(pm2vm)

            print(outMigration % 0)

            opAdd = opAdd[np.argsort(opAdd[:, ADD_INDEX])]
            for i in range(todayReq):
